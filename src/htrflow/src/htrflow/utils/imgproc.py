...
def crop_chatgpt(img: np.ndarray, bbox, padding: int = 0) -> np.ndarray:
    x1, y1, x2, y2 = bbox
    cropped = img[y1:y2, x1:x2]

    pad_x = max(0, (x2 - x1) - cropped.shape[1])
    pad_y = max(0, (y2 - y1) - cropped.shape[0])
    ...
    if pad_x or pad_y:
        if cropped.ndim == 2:
            ...
            raise ValueError(f"Unsupported ndim for crop padding: {cropped.ndim}")

        cropped = np.pad(cropped, pad_width, mode="constant", constant_values=padding)
